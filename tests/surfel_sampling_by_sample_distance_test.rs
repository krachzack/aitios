...
#[test]
fn surfel_sampling_by_sample_distance_test() {
    let directory = common::prepare_test_directory("sampling-by-sample-distance");

    let obj_file = "multi-weathered-sampling-distance.obj";
    let mtl_file = "multi-weathered-sampling-distance.mtl";
    ...
    hit_map_path.push("interacted_surfels_by_distance");
    hit_map_path.set_extension("obj");

    let mut surfels_path = directory.clone();
    surfels_path.push("surfels_by_distance");
